...
wire ale_i_sync, we_sync, oe_sync;
wire [23:0] ad_sync;
...
(*mark_debug="true"*) reg addr_inc, we_last, oe_last, ale_last;
reg we_fall, oe_fall;
reg we_rise, oe_rise;
reg we_o_dly;
...
always @(posedge clk) begin : proc_addr
    if(rst) begin
        addr_o <= 0;
        addr_inc <= 0;
    end else if(ale_i_sync && we_fall) begin
        addr_o <= ad_sync[22:0];
        addr_inc <= ad_sync[23];
...
always @(posedge clk) begin : proc_ctl
    if(rst) begin
        ...
        we_o_dly <= we_sync | ale_i_sync;
        we_o_n <= we_sync | ale_i_sync | we_o_dly;
        oe_o_n <= oe_sync;
    end
end
